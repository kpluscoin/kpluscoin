...
{
    // return if URI is not valid or is no kplus: URI
...
{
    // Convert kplus:// to kplus:
    //
    //    Cannot handle this later, because kplus:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open kplus.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "KPCcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for KPCcoin.lnk
...
{
    return GetAutostartDir() / "kplus.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a kplus.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=KPCcoin\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the kplus app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add kplus app to startup item list
