...
KPLUS now supports full decentralized budgets that are paid directly from the blockchain via superblocks once per month.
...
mnbudget prepare \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<kplus_address\> \<monthly_payment_kplus\> [use_ix(true|false)]
...
mnbudget submit \<proposal-name\> \<url\> \<payment_count\> \<block_start\> \<kplus_address\> \<monthly_payment_kplus\> \<collateral_hash\>
 ...
 * vote-many          - Vote on a KPLUS initiative
 * vote-alias         - Vote on a KPLUS initiative
 * vote               - Vote on a KPLUS initiative/budget
